# AGENTS.md — Gluetun Working Guidelines

## How to work
- Work in small iterations — one focused step at a time.
- Commit after each successful step. Small, frequent commits; never one big commit at the end.
- Re-review the user's prompt before finishing and confirm every request was addressed. If something was deferred, state it clearly in the progress report summary.

## Code quality
- Keep code clean, clear, compact, and consistent. Remove dead/obsolete code, update stale comments, refactor, and reuse shared components/hooks at every iteration.
- Use modern techniques and best practices.
- No need for backward compatibility — it's OK to make migrations and break old formats when it improves the code.
- You may update packages/dependencies to their latest versions and install new tools when needed.
